...
Данная система использует библиотеку Google OR-Tools для создания оптимизированного школьного расписания на основе входных данных из Excel-файла.
...
7. `main.py` - основной скрипт для запуска приложения
...
```bash
pip install pandas numpy openpyxl ortools matplotlib
```
...
- `--output visualiser/optimized_schedule.xlsx` - путь к выходному Excel-файлу (по умолчанию: visualiser/optimized_schedule.xlsx)
- `--time-limit 300` - ограничение времени оптимизации в секундах (по умолчанию: 300)
- `--time-interval 15` - интервал времени для планирования в минутах (по умолчанию: 15)
